...
module TwilioSegment
  class Analytics
    describe Worker do
      before do
        TwilioSegment::Analytics::Transport.stub = true
      end

      describe '#init' do
        it 'accepts string keys' do
          queue = Queue.new
          worker = TwilioSegment::Analytics::Worker.new(queue,
                                                  'secret',
                                                  'batch_size' => 100)
      ...
      describe '#run' do
        before :all do
          TwilioSegment::Analytics::Defaults::Request::BACKOFF = 0.1
        end

        after :all do
          TwilioSegment::Analytics::Defaults::Request::BACKOFF = 30.0
        end

        it 'does not error if the request fails' do
          expect do
            TwilioSegment::Analytics::Transport
              .any_instance
              .stub(:send)
              .and_return(TwilioSegment::Analytics::Response.new(-1, 'Unknown error'))

            queue = Queue.new
            queue << {}
            worker = TwilioSegment::Analytics::Worker.new(queue, 'secret')
            worker.run

            expect(queue).to be_empty

            TwilioSegment::Analytics::Transport.any_instance.unstub(:send)
        ...
        it 'executes the error handler if the request is invalid' do
          TwilioSegment::Analytics::Transport
            .any_instance
            .stub(:send)
            .and_return(TwilioSegment::Analytics::Response.new(400, 'Some error'))
      ...
      describe '#is_requesting?' do
        it 'does not return true if there isn\'t a current batch' do
          queue = Queue.new
          worker = TwilioSegment::Analytics::Worker.new(queue, 'testsecret')
        ...
        it 'returns true if there is a current batch' do
          TwilioSegment::Analytics::Transport
            ...
            .stub(:send) {
              sleep(0.2)
              TwilioSegment::Analytics::Response.new(200, 'Success')
            }
